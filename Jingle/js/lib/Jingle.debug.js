...
J.Page = (function($){
    ...
    var loadSectionTpl = function(hash,callback){
        var param = {},query,replaceSection = false;//是否第一次打开某个页面
        if($.type(hash) == 'object'){
            param = hash.param;
            query = hash.query;
            hash = hash.tag;
        ...
        id = id.replaceAll("_","/");
        var scriptUrl = J.settings.baseScriptPath+id+".js";
        var addJsModule = function(){
    	    try{
	        	seajs.use(scriptUrl,function(moudle){
	        		moudle.init();
	        		module.bindPageEvent();
	        	});	
        	}catch(e){
        		J.Toast.show("error","加载"+scriptUrl+"模块失败!");
        	}
        }
        //已经存在则直接跳转到对应的页面
        if($(hash).length == 1){
            if(q == query){
            	if($(hash).attr('data-refresh') === "true")
            	{
            		addJsModule();
            	}
                callback();
        ...
        var viewUrl = J.settings.baseViewPath+id+J.settings.basePageSuffix;
        loadContent(viewUrl,param,function(html){
        	addJsModule();
...
J.Router = (function($){
    ...
    var _initIndex = function(){
        ...
        if(targetHash != '' && targetHash != indexHash){
        	...
        	//加载初始页面模块JS文件
        	var scriptUrl = J.settings.baseScriptPath+configuration.defaultPage+".js";
        	try{
	        	seajs.use(scriptUrl,function(moudle){
	        		moudle.init();
	        		module.bindPageEvent();
